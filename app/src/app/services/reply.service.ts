...
export class ReplyService {
  ...
  newimg: any =  "assets/default-profil-picture.jpg";
  ...
  ngoninit() {
    ...
    if (this.username) {
      ...
      this.http.get<UserProfile>(this.apiUrl + `getuser/${this.username}`).subscribe(
        data => {
  ...
  loadProfilePhoto() {
    if (this.userProfile) {
      ...
      this.http.get<any>(this.apiUrl + `getuser/${this.username}`).subscribe(
        userData => {
